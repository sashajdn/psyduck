...
use super::{ACCUMULATORS, SimdVector};
    ...
    QuantizedFp + Sized
{
    ...
    #[inline(always)]
    fn accumulate_chunk(a: &[Self; LANES], b: &[Self; LANES], accumulator: &mut Self::Vector) {
        let a_simd = Self::Vector::from_array(a);
        let b_simd = Self::Vector::from_array(b);
        a_simd.fma_accumulate(b_simd, accumulator);
    }
...
impl<const LANES: usize> SimdDotProduct<LANES, ACCUMULATORS> for f32 {
    ...
    fn simd_dot_product(a: &[Self], b: &[Self]) -> Self {
        ...
        for (a_group, b_group) in a_groups.iter().zip(b_groups) {
            Self::accumulate_chunk(&a_group[0], &b_group[0], &mut accumulator_0);
            Self::accumulate_chunk(&a_group[1], &b_group[1], &mut accumulator_1);
            Self::accumulate_chunk(&a_group[2], &b_group[2], &mut accumulator_2);
            Self::accumulate_chunk(&a_group[3], &b_group[3], &mut accumulator_3);
            Self::accumulate_chunk(&a_group[4], &b_group[4], &mut accumulator_4);
            Self::accumulate_chunk(&a_group[5], &b_group[5], &mut accumulator_5);
            Self::accumulate_chunk(&a_group[6], &b_group[6], &mut accumulator_6);
            Self::accumulate_chunk(&a_group[7], &b_group[7], &mut accumulator_7);
        }
        ...
        for (a_chunk, b_chunk) in a_chunk_remainder.iter().zip(b_chunk_remainder) {
            Self::accumulate_chunk(a_chunk, b_chunk, &mut remainder_accumulator);
        }
        ...
        if !a_scalar_remainder.is_empty() {
            ...
            Self::accumulate_chunk(&padded_a, &padded_b, &mut remainder_accumulator);
        }
